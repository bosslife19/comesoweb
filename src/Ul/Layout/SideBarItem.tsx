...
import { BsFillHouseAddFill, BsMicrosoftTeams } from "react-icons/bs";
...
// import pepcode from "../../assets/pepcode.png"
interface SidebarItem {
  text: string; 
  link: string;
  icon: React.ComponentType<{ color: string }>;
  subItems?: SidebarItem[]; // Make subItems optional
}
...
const SideBarItem: React.FC<OnClick> = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  const [activeMainIndex, setActiveMainIndex] = useState<number>(
  ...
  const handleMainItemClick = (mainIndex: number) => {
    setActiveMainIndex(mainIndex);
    setActiveSubIndex(0); // Reset sub-index when main item is clicked
    localStorage.setItem("activeMainIndex", mainIndex.toString());
    localStorage.removeItem("activeSubIndex"); // Remove sub-index from storage
  };
  ...
  const renderSubNav = (mainIndex: number) => {
    // const isActive = activeMainIndex === mainIndex && allItems[mainIndex].subItems
            ...
            >
              <div
                className={`flex items-center justify-start gap-2 font-normal py-4 px-10 rounded-lg ${
                  activeSubIndex === subIndex ? "text-blue" : "text-[#6B788EDF]"
                }`}
              >
                <subItem.icon
                  color={activeSubIndex === subIndex ? "#0A2EE2" : "#6B788EDF"}
                />
  ...
  return (
    ...
    <aside
      ref={sidebar}
      className={`absolute  left-0 top-0 flex h-screen shadow w-[250px] z-[999] border-r lg:z-0 flex-col scroll-container overflow-x-hidden overflow-y-hidden bg-[#FDFDFD] duration-300 ease-linear  lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
    >
      ...
      <div className="flex items-center justify-between gap-2 px-2 py-5.5 lg:py-2.5 ">
        <Link to="dashboard">
        <h3>LOGO</h3>
        {/* <img src={pepcode} alt="Pepcode" className="object-cover mt-10 h-[50px]"/> */}
        </Link>
  ...
  <div className="py-3">
    {allItems.map((item, mainIndex) => (
      <div
        key={mainIndex}
        className={`${
          item.text === "Payout" ? "md:pb-[50px] border-b md:mb-[20px]" : // Add large margin after 'Payout'
          item.text === "Team" ? "md:mb-[20px] md:pb-[100px] border-b" : // Add smaller margin after 'Purchase'
          ""
        }`}
      >
        <div
        ...
        >
          ...
          >
            <item.icon
              color={activeMainIndex === mainIndex ? "#fff" : "#333333"}
            />
